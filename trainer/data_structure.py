    ...
    def update(self, depth, conf, proj_matrix):
        if self.depths is None:
            ...
            for stage in self.depths.keys():
                self.depths[stage] = torch.cat((depth[stage].unsqueeze(1), self.depths[stage][:, :self.max_size, ...]), dim=1)
                self.confs[stage] = torch.cat((conf[stage].unsqueeze(1), self.confs[stage][:, :self.max_size, ...]), dim=1)
