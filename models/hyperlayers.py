...
import functools
from models.module import FCBlock
...
class BatchLinear(nn.Module):
    ...
    def forward(self, inputs):
        output = inputs.matmul(self.weights.permute(*[i for i in range(len(self.weights.shape)-2)], -1, -2))
        output += self.biases
        return output
...
class HyperLinear(nn.Module):
    ...
    def __init__(self,
        ...
        self.hypo_params = FCBlock(in_features=hyper_in_ch,
                                   ...
                                   out_features=(in_ch * out_ch) + out_ch, outermost_linear=True)
